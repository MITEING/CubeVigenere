...
つまり、[平文をkeyでVigenere暗号化 -> 平文最初のn文字をkeyとして暗号化]

このとき、最初のn文字は単に二倍されてしまうので、alphabetが偶数個しかない状態だと復号できない。空白を入れることでこれを回避。
代替案として、最初のn文字は通常のkeyのみで暗号化するのでもよい。空白を一文字と考えるのは無理になることがありそうではある。

また、もう一つのアイデアとして、和ではなくて積を使うことを考えていて、そのためには剰余体をなしていて欲しい。
...
def Main():
    ...
    message = ["Mankind in its present state has been around for a quarter of a million years, yet only the last 4,000 have been of any significance.",
    ...
    for i in range(len(message)):
        message[i] = re.sub(r'[.,!?\']', "", message[i]).replace("-", "")    #erase symbol
        code.append(CodeMessage(message[i], key, N))
